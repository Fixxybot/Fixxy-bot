...
def puto(bot, update):
	...
	if "MEJOR ROM" in recibe:
			bot.send_message(chat_id=update.message.chat_id, text="*No hay ninguna rom mejor que otra .-.*", parse_mode=telegram.ParseMode.MARKDOWN, reply_to_message_id=update.message.message_id)
...
def id(bot, update):
	...
	#if is_group:
	bot.send_message(chat_id=update.message.chat_id, text="*ID del grupo:* " + str(chat_id) + "\n*Tu ID: *" + str(user_id) + "\nUsername: @" + str(username) + "\nNombre: " + str(first_name) + "\nApellido: " + str(last_name) + "\nLenguaje:" + str(code) + "\nBot: " + str(is_bot) , parse_mode=telegram.ParseMode.MARKDOWN, reply_to_message_id=update.message.message_id)
	#else:
...
def callback(bot, update):
	...
	if update.callback_query.data == "720":
			...
			bot.sendDocument(chat_id=query.message.chat_id, document="BQADBAADngMAAtWSWVN2fjCjbiY91QI")
	if update.callback_query.data == "722":
			bot.edit_message_text(text="TWRP para el 722:", chat_id=query.message.chat_id,
				  message_id=query.message.message_id)
			bot.sendDocument(chat_id=query.message.chat_id, document="BQADBAADnQMAAtWSWVNtJ_s7PvzskwI")
...
def hola(bot, update):
	bot.send_message(chat_id=update.message.chat_id, text=random.choice(
		["*Hola*, cuanto tiempo :D", "*Hola*", "Eyyy", "Hacia mucho tiempo que no te veia",
		 "*Holaaa*!!! Cuanto tiempo tio?!?", "Hace mucho que no te veo."]), parse_mode=telegram.ParseMode.MARKDOWN, reply_to_message_id=update.message.message_id)
			...
			if update.message.reply_to_message is not None:
				...
				if prev_user:
					try:
						...
						bot.send_message(chat_id=chat_id, text="*No puedo banear administradores.*", parse_mode=telegram.ParseMode.MARKDOWN, reply_to_message_id=update.message.message_id)
				else:
					bot.send_message(chat_id=chat_id, text="*No has referido ningun usuario.*", parse_mode=telegram.ParseMode.MARKDOWN, reply_to_message_id=update.message.message_id)
			else:
				try:
					if user_id[1]:
						try:
							...
							bot.send_message(chat_id=chat_id, text="*No puedo banear admins.*", parse_mode=telegram.ParseMode.MARKDOWN, reply_to_message_id=update.message.message_id)
				except:
					bot.send_message(chat_id=chat_id, text="*Mal ejecutado, responde a un mensaje, o utiliza:*\n*/ban <user_id>.*", parse_mode=telegram.ParseMode.MARKDOWN, reply_to_message_id=update.message.message_id)
		else:
			bot.send_message(chat_id=chat_id, text="*No eres admin, y me estas diciendo lo que tengo que hacer -.-.*", parse_mode=telegram.ParseMode.MARKDOWN, reply_to_message_id=update.message.message_id)
	else:
		bot.send_message(chat_id=chat_id, text="*Comando solo válido para grupos.*", parse_mode=telegram.ParseMode.MARKDOWN, reply_to_message_id=update.message.message_id)
...
def twrp(bot, update):
	bot.send_message(chat_id=update.message.chat_id, text="Selecciona tu dispositivo: ", parse_mode=telegram.ParseMode.MARKDOWN,
	reply_markup=telegram.InlineKeyboardMarkup(
		[
			[ telegram.InlineKeyboardButton('📲LePro3', callback_data='Le3'), telegram.InlineKeyboardButton("📲LeMax2", callback_data='Le2') ],
			[ telegram.InlineKeyboardButton("❌Cerrar menú", callback_data="Cerrar")]
...
def efbot(bot, update):
	...
	for x in prohibited:
		if x in mensaje:
				...
				bot.delete_message(chat_id=update.message.chat_id, message_id=update.message.message_id)
				bot.send_message(chat_id=-1001122754147, text="*USERNAME:* @{}\n*ID:* {}\n*Ha hecho spam en el chat:* {}".format(username, id, chat_title), parse_mode=telegram.ParseMode.MARKDOWN)
...
aosip = CommandHandler("roms", aosip)
